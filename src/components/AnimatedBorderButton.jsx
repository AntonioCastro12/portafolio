export const AnimatedBorderButton = ({ children, className = "", ...props }) => {
  return (
    <button
      type="button"
      className={`relative bg-transparent border border-border text-foreground hover:border-primary/70 transition-all duration-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary disabled:opacity-50 disabled:cursor-not-allowed group px-8 py-4 text-base md:text-lg font-semibold rounded-full overflow-visible animated-border ${className}`}
      {...props}
    >
      <svg
